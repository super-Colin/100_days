// only supports square (n x n) grid right now
const grid_size = 4;
...
function reset_game_state(state_array, grid_size){
    new_state_array =[];
    if(state_array === []){
        new_state_array = [...state_array];
    }
    array_spots = grid_size * grid_size;
    for (i = 0; i < array_spots; i++) {
        new_state_array[i] = 0;
    }
    console.log('new state array is:');
    console.log(new_state_array);
    return new_state_array
}

function reset_tiles(state_array, tile_array){
    let classes = ['tile'];
    let i;
    for(i=0; i< state_array.length; i++){
        if (state_array[i] != 0) {
            classes[1] = 'tile-' + state_array[i];
            console.log(classes);
        }
        tile_array[i].classList = '';
        tile_array[i].classList.add(...classes);
    }
}

game_state = reset_game_state(game_state, grid_size);

function button_reset() {
    game_state = reset_game_state(game_state, grid_size);
    reset_tiles(game_state, tiles);
}




console.log('game_state array is:');
