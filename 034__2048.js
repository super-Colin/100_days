...
function check_for_move(input_array){
    ...
    for(i=0; i < input_array.length; i++){
       if(input_array[i] == 0){
           moves = true;
        console.log('checkForMoves() found move at index ' + i);
       }
    }
    return moves;
}
...
function sweep_left() {
    ...
    for (i = 0; i < grid_size; i++) {
        ...
        console.log('---checking for possible row moves---')
        // check for moves returns true or false
...
function make_moves(input_array){
    let return_array = [];
    return_array.push(input_array.filter(
        (input) => {
            if(input !== 0){
                return_array.push()
...
function update_game_state(state = game_state){
